...
func TestIface(t *testing.T) {
	...
	for i, test := range tests {
		...
		if jsonutil.MarshalJSONOrDie(cpy) != jsonutil.MarshalJSONOrDie(test.expected) {
			t.Errorf("%d: expected copy to be %#v, got %#v", i, test.expected, cpy)
		}
	}
}
...
func TestCanSet(t *testing.T) {
	...
	cpy := Iface(tst)
	if jsonutil.MarshalJSONOrDie(cpy) != jsonutil.MarshalJSONOrDie(expected) {
